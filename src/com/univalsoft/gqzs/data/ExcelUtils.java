...
public class ExcelUtils {
	...
	public static void setExcelName(String excelName) {
		ExcelUtils.excelName = excelName;
	}
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				for (int j = 0; j < objList.size(); j++) {
					Customers customers = (Customers) objList.get(j);
					sheet.addCell(new Label(0, j + 1, customers.getId(),
							arial12format));
					sheet.addCell(new Label(1, j + 1, customers.getName(),
							arial12format));
					sheet.addCell(new Label(2, j + 1, customers.getAddress(),
							arial12format));
					sheet.addCell(new Label(3, j + 1, customers.getMobile(),
							arial12format));
					sheet.addCell(new Label(4, j + 1, customers.getPhone(),
							arial12format));
					sheet.addCell(new Label(5, j + 1, customers.geteMail(),
							arial12format));
					sheet.addCell(new Label(6, j + 1, customers.getWhatapp(),
							arial12format));
					sheet.addCell(new Label(7, j + 1, customers.getOpening(),
							arial12format));
				}
