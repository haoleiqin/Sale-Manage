...
public class SaleExcelUtils {
	...
	public static void setExcelName(String excelName) {
		SaleExcelUtils.excelName = excelName;
	}
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				for (int j = 0; j < objList.size(); j++) {
					SaleBean customers = (SaleBean) objList.get(j);
					sheet.addCell(new Label(0, j + 1, customers.getId(),
							arial12format));
					sheet.addCell(new Label(1, j + 1, customers.getDate(),
							arial12format));
					sheet.addCell(new Label(2, j + 1, customers.getCustom(),
							arial12format));
					sheet.addCell(new Label(3, j + 1, customers.getProduct(),
							arial12format));
					sheet.addCell(new Label(4, j + 1, customers.getNotes(),
							arial12format));
					sheet.addCell(new Label(5, j + 1, customers.getPrice(),
							arial12format));
				}
