...
public class ProExcelUtils {
	...
	public static void setExcelName(String excelName) {
		ProExcelUtils.excelName = excelName;
	}
		...
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				for (int j = 0; j < objList.size(); j++) {
					ProuctsBean customers = (ProuctsBean) objList.get(j);
					sheet.addCell(new Label(0, j + 1, customers.getId(),
							arial12format));
					sheet.addCell(new Label(1, j + 1, customers.getName(),
							arial12format));
					sheet.addCell(new Label(2, j + 1, customers.getUnit(),
							arial12format));
					sheet.addCell(new Label(3, j + 1, customers.getPurcharse(),
							arial12format));
					sheet.addCell(new Label(4, j + 1, customers.getSale(),
							arial12format));
					sheet.addCell(new Label(5, j + 1, customers.getCategory(),
							arial12format));
				}
